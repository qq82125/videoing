...
- 快速把行业选题变成短视频脚本和首版产物
- 需要频繁比较标题、封面和口播表达
- 需要先低成本试产，再决定要不要继续做镜头和出片
- 希望保留完整草稿资产，方便继续生产而不是重来一遍
...
如果你本机之前跑过旧服务，建议先停掉旧进程，避免误以为页面没更新。
...
如果只是先把本地流程跑起来，最小配置通常只需要：

- `OPENAI_API_KEY`
- `OPENAI_BASE_URL`（如果走代理）
- `ffmpeg`

没有远程图片或配音能力时，项目会尽量回退到本地兜底，不会整条流程直接断掉。
